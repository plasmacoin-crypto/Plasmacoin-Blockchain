        ...
        if (i < 16) { // Chunks 1-16
            ...
            for (unsigned int j = 0; j < blocks[i].size(); j += 8) {
                blockstr += (
                    ...
                    //
                    sigma_1(bitset<8>(blocks[i - 2].to_string().substr(j, j + 8))).to_string() + \
                    blocks[i - 7].to_string().substr(j, j + 8) + \
                    sigma_0(bitset<8>(blocks[i - 15].to_string().substr(j, j + 8))).to_string() + \
                    blocks[i - 16].to_string().substr(j, j + 8)
                );
            }
        }
    }
